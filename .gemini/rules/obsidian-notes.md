...
Four native MCP tools are registered and pre-approved. You MUST use one before `read_file`-ing any vault note.
...
- "I'm looking for notes on a topic" → `vault_find` → then `vault_show` on the best hit → then `read_file`
